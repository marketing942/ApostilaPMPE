...
**Modelo em uso: B** (o site dispara o Lead), em [script.js](script.js):

```js
const LEAD_MODE = "site";   // "site" (Modelo B) | "painel" (Modelo A)
```

Esta é uma landing de **venda direta** (Resumo Bizurado, R$ 59,90) — o formulário
...
Auditado item a item, sem alterações necessárias:

- Barreira de submit em **fase de captura** no `document`, com
  `stopImmediatePropagation` ([§7.8](#78-ordem-de-registro-dos-listeners-de-submit))
- Validação de telefone por dígitos, removendo o `+55` pelo `+` literal ([§7.7](#77-validar-o-telefone-pelo-length-da-string--o-erro-mais-traiçoeiro))
- `toE164()` antes do `send_event`; e-mail normalizado para minúsculas
- `waitForPixel()` antes de disparar ([§8.5](#85-corrida-com-o-start-assíncrono))
- Sem `form.reset()` antes do redirect ([§7.6](#76-formreset-e-redirecionamento-cedo-demais))
- `pxa_mask_phone` e `name` casando com as keywords ([§6](#6-como-a-pixelx-identifica-os-campos))
- **Nenhum** `fbq`, **nenhum** loader direto da PixelX ([§3.2](#32-existem-dois-jeitos-de-a-pixelx-entrar-na-página), [§4](#4-inventário-dos-5-emissores-de-lead))
- `noscript` sem espaço depois de `id=` ([§3.3](#33-o-noscript-do-gtm))
- Exit popup com `stopSubmitPropagation: true` — o cadastro na comunidade
  **não** vira Lead de venda

### ⚠️ O risco que o código não alcança

O `<button>` de submit carrega o id opaco da PixelX:

```html
<button type="submit" class="cta" id="IPEyzyfmJhKQEYIXAlZH">
```

Pelo [§6](#6-como-a-pixelx-identifica-os-campos), esse id identifica o
**`<form>`** no painel, não o botão. Consequências:

| Tipo de regra no painel | Situação |
|---|---|
| Regra de **submit** vinculada a esse id | Não encontra formulário nenhum — inerte |
| Regra de **clique** nesse elemento | **Dispara, e o site não tem como impedir** |

O clique acontece **antes** do submit, então nenhuma barreira o alcança. Se
existir regra de clique ativa para esse id, ela soma com o nosso `send_event`.
Confira no painel — teste de [§10.3](#103-auditoria-de-emissores-duplicados).

> Este mesmo id opaco está em uso também nas landings **PMPE** e
> **captura-cppem**. Três páginas diferentes reportando para o mesmo cadastro de
> formulário no painel — vale revisar se é intencional.

### Verificação automatizada

**13 checagens** em Chrome real, com uma PixelX falsa contando cada `send_event`
e espiões simulando regras de painel no `<form>` e no `<button>`:

- submit inválido → zero Lead, não chega ao `<form>`
- submit válido → **exatamente 1** `send_event`, com `Lead`, E.164 e e-mail minúsculo
- envio repetido → continua **1** Lead, **1** redirect e **1** `iniciar_checkout`
- regra de submit do painel → não alcança o formulário
- exit popup → **zero** Lead, e regra externa não o alcança
...
- [index.html](index.html) — loader do GTM, `#lead-form` no modal, `#xp-modal`
- [script.js](script.js) — `LEAD_MODE`, `trackLead()`, `waitForPixel()`,
  `toE164()`, guarda `envioEmAndamento` e a barreira de submit
- [exit-popup.js](exit-popup.js) — kit portátil, `stopSubmitPropagation: true`
